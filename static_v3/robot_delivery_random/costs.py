...
Перенесено из static_v2/robot_delivery_static/costs.py БЕЗ ИЗМЕНЕНИЙ (а туда --
из dynamic_v2): правила маршрутизации и стоимости операции у всех трёх моделей
одни и те же, различаются только моменты, в которые эта оценка вызывается.

В static_v3 она вызывается РОВНО ОДИН РАЗ на доставку -- на Шаге 4
(scheduler.py), от ФАКТИЧЕСКИХ позиций роботов на начало раунда и с
накопленным глобальным built. Это и есть реальная стоимость исполнения.

Отличие от static_v2: там та же процедура вызывалась ещё и на Шаге 3, чтобы
построить таблицу "пара x груз" и распределить грузы по приоритетам. В
static_v3 и пары, и распределение случайны, никакой предварительной оценки не
требуется, поэтому таблицы нет вовсе, а эвристической оценкой работы модели
служит итоговая фактическая стоимость Шага 5 (см. README).
