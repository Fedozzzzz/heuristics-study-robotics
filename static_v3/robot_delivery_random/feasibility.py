"""
Шаг 0. Проверка достижимости решения (перенесено из static_v2 без изменений:
формулировка шага у всех трёх моделей идентична).

Постановка static_v3 формулирует его как перебор подграфов: "для каждого
подграфа, состоящего из уже связанных островов, проверяется -- (1) все ли точки
доставки входят в данный подграф; (2) если нет, можно ли его потенциально
связать с другими подграфами за счёт строительства мостов так, чтобы все
операции доставки могли быть выполнены; (3) есть ли в результирующем подграфе
хотя бы один доставщик и хотя бы один строитель". Проверки (1) и (2) сводятся
к одной проверке на G' -- см. ниже.
