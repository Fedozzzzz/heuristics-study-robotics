"""
Эксперимент: сравнение двух эвристик приоритета груза (Шаг 1) на ОДНИХ И ТЕХ
ЖЕ случайных сценариях -- direct (p = W_C, "дороже маршрут -- выше приоритет")
vs inverse (p = 1/W_C, "дешевле маршрут -- выше приоритет").

Строит три графика:
  1. compare_real_vs_ncargos.png -- средняя РЕАЛЬНАЯ стоимость (real) для
     обеих эвристик в зависимости от числа грузов, на одном поле.
  2. compare_real_vs_estimated_bars.png -- для каждой эвристики: её реальная
     ...
     prognosis).
  3. compare_win_rate.png -- доля сценариев, где эвристика дала минимальную
     реальную стоимость (только полностью доставленные сценарии).

ЗАПУСК:
    python experiments/compare_direct_vs_inverse.py \\
        --n-islands 18 --n-pairs 3 -n 24 --n-runs 15 --seed 0

Список параметров:
    python experiments/compare_direct_vs_inverse.py --help
"""
...
DEFAULT_HEURISTICS = ["direct", "inverse"]


def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Сравнение эвристик приоритета груза direct vs inverse на одних и тех же сценариях."
    )
    ...
    p.add_argument("--output-dir", default=None,
                    help="Куда сохранять CSV и графики (по умолчанию -- "
                         "../outputs/direct_vs_inverse относительно этого файла).")
    return p
...
def main(argv=None) -> int:
    ...
    output_dir = args.output_dir or os.path.join(
        os.path.dirname(__file__), "..", "outputs", "direct_vs_inverse"
    )
